...
class SynergyMonitor {
    ...
    async start() {
        ...
        // Test DB connectivity before starting
        // Temporarily suppress stderr to hide Prisma's C++ error output
        ...
        try {
            ...
            process.stderr.write = originalStderrWrite; // restore
        }

        if (!dbAvailable) {
            console.warn('⚠️  Synergy Monitor: DB unavailable, skipping (demo mode)');
            return;
        }
    ...
    async scanForOpportunities() {
        try {
            ...
            if (error.message?.includes('localhost:5432') || error.message?.includes('database')) {
                console.warn('⚠️  Synergy Monitor: DB unavailable, skipping scan (demo mode)');
            } else {
                console.error('❌ Synergy scan error:', error.message);
            }
        }
    }
